	...
	class AxyQuattrok : Units.Unit
	{
		...
		public override byte[] getConf()
		{
			...
			try
			{
				...
				//getCoeffs();
				//conf[30] = (byte)(coeffs[1] >> 8);
				//conf[31] = (byte)(coeffs[1] & 0xff);
				//conf[32] = (byte)(coeffs[2] >> 8);
				//conf[33] = (byte)(coeffs[2] & 0xff);
				//conf[34] = (byte)(coeffs[3] >> 8);
				//conf[35] = (byte)(coeffs[3] & 0xff);
			}
			catch
			{
				throw new Exception(unitNotReady);
		...
		public unsafe override void download(string fileName, uint fromMemory, uint toMemory, int baudrate)
		{
			...
			fo.Write(new byte[] { model_axyQuattrok, (byte)254 }, 0, 2);
